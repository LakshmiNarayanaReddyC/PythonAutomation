...
try:
    if Fetch_Desired_Capabilities.confirm_platform() != "Android":
        ...
        driver = webdriver.Remote('http://localhost:4723/wd/hub', IOS_Capabilities.relaunch_ios_app())
    else:
        # Connecting to APPIUM SERVER with default port
        # http://localhost:4723/wd/hub
        driver = webdriver.Remote('http://localhost:4723/wd/hub', Android_Capabilities.relaunch_android_app())
